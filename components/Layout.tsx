    ...
    <div className='min-h-screen flex flex-col md:flex-row bg-slate-50 dark:bg-slate-900 transition-colors'>
      ...
      >
        <div className='p-8'>
          ...
          >
            <div className='w-10 h-10 bg-brand-600 rounded-xl flex items-center justify-center text-white font-black text-2xl shadow-xl shadow-brand-500/30'>
              T
            </div>
        ...
        <nav className='flex-1 overflow-y-auto px-6 space-y-1 custom-scrollbar'>
          <p className='px-3 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 opacity-70'>
            Categories
          </p>
        ...
        <div className='p-6 bg-slate-50 dark:bg-slate-900/50 m-4 rounded-3xl border border-slate-100 dark:border-slate-700 space-y-5 shadow-inner'>
          <p className='text-[10px] font-black text-brand-600 dark:text-brand-400 uppercase tracking-widest px-1'>
            Global Configuration
          </p>

          <div className='space-y-2'>
            <label className='block text-xs font-bold text-slate-500 dark:text-slate-400 px-1'>
              Local Currency
            </label>
      ...
      <main className='flex-1 flex flex-col min-w-0 overflow-x-hidden'>
        ...
        <header className='md:hidden flex items-center justify-between p-4 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-40'>
          <button
            onClick={toggleSidebar}
            className='p-2 text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-700 rounded-xl'
          >
            <svg
              className='w-6 h-6'
              fill='none'
          ...
          <div className='flex items-center gap-2'>
            <div className='w-6 h-6 bg-brand-600 rounded-lg flex items-center justify-center text-white font-black text-xs'>
              T
            </div>
            <span className='font-black dark:text-white tracking-tighter text-lg uppercase'>
              ToolVerse
            </span>
          </div>
          <div className='text-xs font-black px-2 py-1 bg-brand-100 text-brand-700 rounded-lg'>
            {currency}
          </div>
        </header>
        ...
        <footer className='mt-auto p-12 text-center border-t border-slate-100 dark:border-slate-700'>
          <p className='text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.4em] mb-4'>
            © {new Date().getFullYear()} ToolVerse Professional utility suite
          </p>
